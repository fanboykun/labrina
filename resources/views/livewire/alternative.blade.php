<div>
    <x-form>
        <x-slot name="title">
            {{ __('Create The Alternative') }}
        </x-slot>

        <x-slot name="description">
            {{ __('The alternative(s) is going to be the subject to get the outrakning') }}
            <br>
            {{ $project_id->name }}
        </x-slot>

        <x-slot name="form">
            <div class="px-4 py-5 mt-5 rounded my-5 bg-white sm:p-6 shadow overflow-hidden">
                <div>
                    <form>
                        <!-- Name -->
                        ...
                        <div wire:loading.attr="disabled" wire:target="addAlternative" class="grid gap-4 grid-cols-{{ $cols }}">
                            ...
                            <div>
                                <x-input wire:model="inputs.{{ $key }}.criterias.{{ $k }}.value" id="value[{{ $key }}][value]" class="block mt-1 w-full" type="number" name="value" placeholder="input value for {{ $v['c_code'] }} || max value = {{ $v['max_value'] }}" required />
                            </div>
                            @endforeach
                            <div class="flex w-full justify-end">
